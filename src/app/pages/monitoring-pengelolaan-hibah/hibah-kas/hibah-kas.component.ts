...
@Component({
  selector: 'ngx-hibah-kas',
  templateUrl: './hibah-kas.component.html',
  styleUrls: ['./hibah-kas.component.scss']
})
export class HibahKasComponent implements OnInit, OnDestroy {

  @ViewChild(PaginationComponent, {static: true}) child: PaginationComponent;
  loadingData: boolean;

  monitorHibahKasList$: Observable<any[]>;
  ...
  constructor(private excelService: ExcelService, private http: HttpClient, private alertService: AlertService, private toastService: NbToastrService, private router: Router, private route: ActivatedRoute) {
    this.settings.column = [
      ...
      { title: 'Tgl Pembukaan Rekening', type: 'text', name: 'tgl_izin_rek', visible: true },
      { title: 'No Rekening', type: 'text', name: 'no_rek', visible: true },
      { title: 'Tgl DIPA', type: 'text', name: 'tgl_izin_dipa', visible: true },
      { title: 'No DIPA', type: 'text', name: 'no_dipa', visible: true },
      ...
      { title: 'Mata Uang SPHL SP3HL', type: 'text', name: 'mata_uang_sphl_sp3hl', visible: true },
      { title: 'Valas SPHL SP3HL', type: 'text', name: 'valas_sphl_sp3hl', visible: true },
      { title: 'Rupiah SPHL SP3HL', type: 'text', name: 'rupiah_sphl_sp3hl', visible: true },
      { title: 'Mata Uang Belanja', type: 'text', name: 'mata_uang_belanja', visible: true },
      { title: 'Valas Belanja Hibah', type: 'text', name: 'valas_belanja_hibah', visible: true },
      { title: 'Rupiah Belanja Hibah', type: 'text', name: 'rupiah_belanja_hibah', visible: true },
    ]
  ...
  ngOnInit() {
    this.getMonitoringHibahBarangJasaList(this.pageNumber);
  ...
  getMonitoringHibahBarangJasaList(pageNumber: number) {
    ...
    let pagedList: any = {};
    this.http.get(Globals.apiService + `/monitoring/kas?search=${this.search}&sort=${this.sort}&limit=${this.pageSize}&offset=${pageNumber}`)
      .pipe(takeUntil(this.unsubscribe$))
      .subscribe((data: any) => {
        this.pageNumber = pageNumber;
        pagedList.TotalPage = Math.ceil(data.total_record / this.pageSize);
        this.monitorHibahKasList$ = of(data.data);
  ...
  getListAllMonitoringKas() {
    this.loadingData = true;
    return this.http.get(Globals.apiService + `/monitoring/allkas`)
  }
